...
- **Trazabilidad:** cada requisito debe enlazar, cuando exista, el/los issue(s) de GitHub (`#N`) que lo trabajan y el módulo (`services/<nombre>`, `packages/contracts`, `.claude/*`, etc.) donde vive su implementación. Los requisitos trabajados en la **Fase 0** ya enlazan su issue de GitHub (RF-005 → #2, RNF-001 → #3, RNF-002 → #4); el resto queda `—` hasta que se abra su issue.
...
| RF-005 | Servicio de fusión multimodal que combina los resultados de las 3 modalidades (mamografía, histopatología, genómica) en un resultado único; la implementación actual usa un promedio simple (mock) con umbral estricto no clínico (`avg > 0.5`), quedando late fusion / attention fusion como trabajo futuro. | Implementado (mock) | `services/fusion` | #2 |
...
| ID | Texto | Estado | Módulo | Issue(s) |
|----|-------|--------|--------|----------|
| RNF-001 | **PHI:** ningún identificador de paciente/caso (`case_ref`), ruta o nombre de archivo DICOM/WSI, `result_json` de predicción, ni URL de Storage sin firmar puede aparecer en logs, `stdout`/`stderr` ni en respuestas de API sin pasar por signed URL generada server-side. **Fase 0 (parcial):** `case_ref` sale de la URL y de la respuesta pública (`POST /analyze` con cuerpo `AnalyzeRequest`; `ClinicalAlert` expone solo un `analysis_id` opaco). Pendientes: autenticación/autorización, redacción centralizada de logs, signed URLs y persistencia de auditoría. | Implementado parcial | `services/gateway`, `packages/contracts` (transversal) | #3 |
| RNF-002 | Meta operativa **aprobada por el director** para la rebanada vertical: **AUC-ROC ≥0.92** sobre el dataset de referencia (CBIS-DDSM), con protocolo de evaluación congelado antes de medir. Se miden y reportan sensibilidad, especificidad y recall como métricas complementarias; no se afirma concordancia BI-RADS si el protocolo/dataset no la habilita. Las metas aspiracionales de la visión inicial del profesor (sens. >95%, esp. >90%, AUC-ROC >0.97, recall >94%, precisión BI-RADS >93%) quedan como referencia de largo plazo, fuera del alcance de validación del TG. Falta registrar la evidencia formal de esta aprobación (ver `docs/alcance-vigente.md`). | Propuesto | `services/mammography` (y futuras modalidades) | #4 |
